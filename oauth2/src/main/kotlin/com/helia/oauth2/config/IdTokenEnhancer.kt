...
class IdTokenEnhancer(private val jwtAccessTokenConverter: JwtAccessTokenConverter) : TokenEnhancer {
    override fun enhance(accessToken: OAuth2AccessToken, authentication: OAuth2Authentication): OAuth2AccessToken {
        if (accessToken.scope.contains(OidcScopes.OPENID)) {
            ...
            idToken.scope = mutableSetOf(OidcScopes.OPENID)
            idToken.refreshToken = null
            (accessToken as DefaultOAuth2AccessToken).additionalInformation = Map.of<String, Any>(OidcParameterNames.ID_TOKEN, jwtAccessTokenConverter.enhance(idToken, authentication).value)
        }
        return accessToken
    }
}